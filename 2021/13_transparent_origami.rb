...
# find the manual; as you go to open it, page 1 falls out. It's a large sheet of transparent
# paper! The transparent paper is marked with random dots and includes instructions
...
# below 0,0. The coordinates in this example form the following pattern, where # is
...
# fold along y=7, which designates the line formed by all of the positions where y is
# 7 (marked here with -):
#
ex4 = <<~T
  ...#..#..#.
...
@data.each do |line|
  ...
  if folds
    _, axis, value = line.match(/.*(\w)=(\d+)/).to_a
    value = value.to_i
    instrs << [axis, value]
  else
...
xmax, ymax = coords.transpose.map { _1.max }

pp coords.sort_by { _1 }

grid = Array.new(ymax) { Array.new(xmax + 1) { 0 } }

def put_grid(g, axis = 'y')
  return
  g = g.transpose if axis == 'x'
  puts g.map.with_index { |r, i| i.to_s.ljust(3, ' ') + r.map { |v| v.zero? ? '.' : '#' }.join }.join("\n")
end

coords.each { grid[_2][_1] = 1 }
# put_grid(grid)
# puts '-' * 25

instrs.each.with_index do |(axis, value), i|
  puts "i=#{i} axis=#{axis} value=#{value} gw=#{grid.first.length} gh=#{grid.length}"
  puts 'grid'
  put_grid(grid)
  puts '-' * 25
  ...
  put_grid([mid], axis)
  top = grid[0...value]
  bottom = grid[value + 1..grid.length]
  pp [value, top.length, bottom.length]
  puts 'top'
  put_grid(top, axis)
  puts '-' * 25
  puts 'bottom'
  put_grid(bottom, axis)
  grid = top.map.with_index do |row, i|
    row.map.with_index do |col, j|
      if col == 1 || bottom.reverse[i][j] == 1
        1
      else
        0
      end
