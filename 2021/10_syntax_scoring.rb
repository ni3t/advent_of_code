...
# (if any) can immediately start. Every chunk must open and close with one of four legal
...
# above.
#
# Examples of corrupted chunks include (], {()()()>, (((()))}, and <([]){()}[{}]). Such
# a chunk can appear anywhere within a line, and its presence causes the whole line
...
# for a line, take the first illegal character on the line and look it up in the following
# table:
#
# - ): 3 points.
# - ]: 57 points.
...
# by considering the completion string character-by-character. Start with a total score
# of 0. Then, for each character, multiply the total score by 5 and then increase the
...
@ex1 = ex2.each_line.map(&:chomp).map(&:chars)
...
@data.map do |line|
  stack = []
  r = line.find { MAP[_1] ? stack.push(_1).then { nil } : MAP[stack.pop] != _1}
  [r, r ? nil : stack.reverse.reduce(0) { (_1 * 5) + SCORES2[MAP[_2]] }]
end.transpose.tap do
  puts(_1.compact.sum { |c| SCORES[c] })
  _2.compact.sort.tap { |a| puts a[a.length / 2] }
end
