...
# all unmarked numbers on that board; in this case, the sum is 188. Then, multiply that
# sum by the number that was just called when the board won, 24, to get the final score,
# 188 * 24 = 4512.
#
...
# will win last and choose that one. That way, no matter which boards it picks, it will
# win for sure.
#
...
class BingoCard
  attr_accessor :board, :last_n, :recorded
  ...
  def to_s
    board.map{ _1.join(' ') }.join("\n")
  end

  def record(n)
    board.each.with_index do |line, i|
      line.each.with_index do |col, j|
        recorded << [i, j] if col == n
        self.last_n = n
      end
    end
  end
  ...
  def finalscore
    board.map.with_index do |line, i|
      line.map.with_index do |col, j|
        recorded.include?([i, j]) ? 0 : col
      end
    end.flatten.sum * last_n
  end
end
...
order = @data.shift.split(',').map(&:to_i)
boards = @data.map { _1.split.map(&:to_i) }.reject(&:empty?).each_slice(5).map { BingoCard.new(_1) }
winners = []

order.each.with_index do |n, _i|
  (boards - winners).map { _1.record(n) }
  round_winners = boards.filter(&:winner?)
  round_winners.each do |w|
    winners << w unless winners.include?(w)
  end
  break if winners.count == boards.count
end

puts winners.first.finalscore
puts winners.last.finalscore

winners.first.tap { |w| pp(w.board + w.board.transpose) }
