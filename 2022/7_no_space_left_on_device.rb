...
# # of each directory. The total size of a directory is the sum of the sizes of the files
...
# # the sum of their total sizes is 95437 (94853 + 584). (As in this example, this process
# # can count files more than once!)
# #
# # Find all of the directories with a total size of at most 100,000. What is the sum of
...
# FFile = Struct.new(:name, :size) do
#   # def pretty_print(pp)
#   #   pp.text "#{name} (#{size})"
#   # end
# end
# FFolder = Struct.new(:name, :contents) do
#   # def pretty_print(pp)
#   #   pp.text "[#{name}]"
#   #   pp.group(pp.current_group.depth + 1) do
#   #     contents.map do |c|
#   #       pp.breakable("\n")
#   #       pp.pp c
#   #     end
...
#   def folders
#     contents.filter { _1.is_a? FFolder }
#   end

#   def files
#     contents.filter { _1.is_a? FFile }
#   end

#   def all_folders
#     folders.concat(
#       folders.map(&:all_folders)
#     ).flatten.uniq(&:name)
#   end
...
# input.each_line.map(&:strip).reject(&:empty?).each do |line|
#   puts "---- #{line} ----"
#   next if line == "$ cd /"
#   if (m = line.match(/\$ cd (\w+)/))
#     @cwd << @cwd.last.contents.find { |ff| ff.name == m[1]}
#   end
#   if line == "$ cd .."
#     @cwd.pop
#   end
...
#   # dir
#   if (m = line.match(/dir\s(\w+)/))
#     @cwd.last.contents << FFolder.new(m[1], [])
#   end

#   # file
#   if (m = line.match(/(\d+)\s(\S+)/))
#     @cwd.last.contents << FFile.new(m[2], m[1].to_i)
#   end
# end

# pp Root
# pp Root.all_folders.map { [_1.name, _1.size, _1.under] }
# pp Root.all_folders.filter(&:under).sum(&:size)
...
dirsizes_nonrec = DATA.each_line(chomp: true).each_with_object({}) { |line, dirsizes|
  case line.split
  in [?$, 'cd', arg]
    lsing = false
    case arg
