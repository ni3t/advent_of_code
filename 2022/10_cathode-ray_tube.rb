...
# too loud to tell exactly what they're saying. They finish crossing the bridge and
# disappear from view.
#
...
# to be some kind of cathode-ray tube screen and simple CPU that are both driven by
# a precise clock circuit. The clock circuit ticks at a constant rate; each tick is
# called a cycle.
#
...
#
# The CPU uses these instructions in a program (your puzzle input) to, somehow, tell
...
# in each row is in position 0, and the right-most pixel in each row is in position
# 39.
#
...
# If the sprite is positioned such that one of its three pixels is the pixel currently
...
T
#
# Render the image given by your program. What eight capital letters appear on your
# CRT?

input = DATA

instructions = input.each_line.map(&:chomp).reject(&:empty?)
